...
//Необходимо удалить всех нечастых покупателей из нашей клиентской базы и обновить записи по оставшимся частым покупателям для предоставления скидок.
...
int main()
{
	...
	for_each(conteiner.begin(), conteiner.end(), output);
	int k = 0;
	for_each(conteiner.begin(), conteiner.end(), [&k] (dataBase i) {k = k + i.kolvo; } );
	k = k / conteiner.size();
	int k1 = k / 2;
	cout << "----------------------------------------------------------------" << endl;
	cout << "Среднее арифметическое значение покупок среди всех покупателей — " << k << ". Все покупатели, имеющие меньше " << k1 << " покупок будут удалены!" << endl;
	cout << "----------------------------------------------------------------" << endl;
	for (int i = conteiner.size() - 1; i >= 0; i--)
	{
		if (conteiner[i].kolvo < k1)
		{
			conteiner.pop_back();
		}
	}
